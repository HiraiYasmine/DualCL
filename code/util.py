...
def load_freebase_triple_data_multimodal(train_triples_file, entity_embeddings_txt,entity_embeddings_img, relation_embeddings):
    ...
    for triple in training_triples:
        ...
        tail = triple[1]
        rel = triple[2]
        if head in entity_embeddings_img and tail in entity_embeddings_img:
...
def get_batch_with_neg_heads_tails_relation_and_infonce_imgs(training_data, triples_set, entity_list, start, end, n, entity_embedding_txt,entity_embedding_img,relation_embeddings, entity_similarity):
    ...
    for triple in batch_data:
        #text_triple = (triple[5], triple[7], triple[6])
        #print("org triple: ", text_triple)
        if triple[5] not in h_entity:
            h_entity.append(triple[5])
        if triple[7] not in t_entity:
            t_entity.append(triple[7])
    
    h_entity = set(h_entity)
    t_entity = set(t_entity)
    ...
    for triple in batch_data:
        ...
        h_infonce_imgs_temp = sample_infonce_txt(text_triple[0], n, entity_embedding_txt, entity_similarity, h_entity)
        t_infonce_imgs_temp = sample_infonce_txt(text_triple[1], n, entity_embedding_txt, entity_similarity, t_entity)
        h_infonce_imgs += h_infonce_imgs_temp
        t_infonce_imgs += t_infonce_imgs_temp
    ...
    return np.asarray(h_data_txt), np.asarray(h_data_img), np.asarray(r_data), np.asarray(t_data_txt), \
           ...
           np.asarray(h_neg_data_txt),\
           np.asarray(h_neg_data_img),\
           np.asarray(h_infonce_imgs),np.asarray(t_infonce_imgs)

def sample_infonce_imgs(entity, n, entity_embedding_img, entity_similarity, entity_to_be_filitered):
    #img_similarity_sorted = set(entity_similarity[entity].copy())
    # for i in range(len(entity_to_be_filitered)):
    #     img_similarity_sorted.remove(entity_to_be_filitered[i])
    #img_similarity_sorted = list(set(entity_similarity[entity].copy()) - entity_to_be_filitered)
    img_similarity_sorted = list(entity_similarity[entity] - entity_to_be_filitered)
    if 2 * n < len(img_similarity_sorted):
        infonce_index_temp = img_similarity_sorted[:2*n]
    else:
        infonce_index_temp = img_similarity_sorted
    
    random.shuffle(infonce_index_temp)
    infonce_index = infonce_index_temp[:n]
    infonce_imgs = [entity_embedding_img[key] for key in infonce_index]
    return infonce_imgs

def sample_infonce_txt(entity, n, entity_embedding_txt, entity_similarity, entity_to_be_filitered):
    #img_similarity_sorted = set(entity_similarity[entity].copy())
    # for i in range(len(entity_to_be_filitered)):
    #     img_similarity_sorted.remove(entity_to_be_filitered[i])
    #img_similarity_sorted = list(set(entity_similarity[entity].copy()) - entity_to_be_filitered)
    ...
    random.shuffle(infonce_index_temp)
    infonce_index = infonce_index_temp[:n]
    infonce_imgs = [entity_embedding_txt[key] for key in infonce_index]
    return infonce_imgs

def load_triple_data(train_triples_file, entity_embeddings, relation_embeddings, entity_list):
    training_intances = []
    ...
    for triple in tqdm(training_triples):
        ...
        rel = triple[2]
        if head in entity_list and tail in entity_list:
            head_embd = entity_embeddings[head]
            tail_embd = entity_embeddings[tail]
            rel_embd = relation_embeddings[rel]
            train_instance = (head_embd, rel_embd, tail_embd, head, rel, tail)
...
def get_batch_with_neg_heads_tails_relation_and_cl_entity(training_data, triples_set, entity_list, start, end, n, entity_embeddings, img_embeddings, txt_embeddings, relation_embeddings, entity_similarity):
    ...
    for triple in batch_data:
        ...
        h_entity.append(triple[3])
        t_entity.append(triple[5])
    ...
    for triple in batch_data:
        # train_instance = (head_embd, rel_embd, tail_embd, head, rel, tail)
        text_triple = (triple[3], triple[5], triple[4])#(h, t, r)

        h_data.append(triple[0])
        h_img_data.append(img_embeddings[text_triple[0]])
        h_txt_data.append(txt_embeddings[text_triple[0]])
        r_data.append(triple[1])
        t_data.append(triple[2])
        t_img_data.append(img_embeddings[text_triple[1]])
        t_txt_data.append(txt_embeddings[text_triple[1]])

        
        t_neg = sample_negative_tail(triples_set, entity_list, text_triple)[1]
        t_neg_embed = entity_embeddings[t_neg]
        t_neg_data.append(t_neg_embed)
        t_neg_img_data.append(img_embeddings[t_neg])
        t_neg_txt_data.append(txt_embeddings[t_neg])

        h_neg = sample_negative_head(triples_set, entity_list, text_triple)[0]
        h_neg_embed = entity_embeddings[h_neg]
        h_neg_data.append(h_neg_embed)
        h_neg_img_data.append(img_embeddings[h_neg])
        h_neg_txt_data.append(txt_embeddings[h_neg])

        h_cl_index = sample_cl_data(text_triple[0], n, entity_similarity, h_entity)
        t_cl_index = sample_cl_data(text_triple[1], n, entity_similarity, t_entity)
        h_cl_data += [entity_embeddings[key] for key in h_cl_index]
        h_cl_img_data += [img_embeddings[key] for key in h_cl_index]
        h_cl_txt_data += [txt_embeddings[key] for key in h_cl_index]
        t_cl_data += [entity_embeddings[key] for key in t_cl_index]
        t_cl_img_data += [img_embeddings[key] for key in t_cl_index]
        t_cl_txt_data += [txt_embeddings[key] for key in t_cl_index]

        if (triple[3], triple[4]) not in triple_h_r:
            triple_h_r.append((triple[3], triple[4]))
            triple_h_data.append(entity_embeddings[triple[3]])
            triple_h_img_data.append(img_embeddings[triple[3]])
            triple_h_txt_data.append(txt_embeddings[triple[3]])
            triple_r_data.append(relation_embeddings[triple[4]])


    return np.asarray(h_data), np.asarray(h_img_data), np.asarray(h_txt_data), np.asarray(r_data),\
           np.asarray(t_data), np.asarray(t_img_data), np.asarray(t_txt_data),\
           np.asarray(t_neg_data), np.asarray(t_neg_img_data), np.asarray(t_neg_txt_data),\
           np.asarray(h_neg_data), np.asarray(h_neg_img_data), np.asarray(h_neg_txt_data),\
           np.asarray(h_cl_data), np.asarray(h_cl_img_data), np.asarray(h_cl_txt_data),\
           np.asarray(t_cl_data), np.asarray(t_cl_img_data), np.asarray(t_cl_txt_data),\
           np.asarray(triple_h_data), np.asarray(triple_h_img_data), np.asarray(triple_h_txt_data), np.asarray(triple_r_data)

def sample_cl_data(entity, n, entity_similarity, entity_to_be_filitered):
    similarity_sorted = list(entity_similarity[entity] - entity_to_be_filitered)
    if 2 * n < len(similarity_sorted):
        index_temp = similarity_sorted[:2*n]
    else:
        index_temp = similarity_sorted
    
    random.shuffle(index_temp)
    cl_index = index_temp[:n]
    #cl_data = [entity_embeddings[key] for key in cl_index]
    return cl_index

def get_batch_with_neg_heads_tails_relation_and_cl_entity_fb(training_data, triples_set, entity_list, start, end, n, entity_embeddings, img_embeddings, txt_embeddings, relation_embeddings, entity_similarity):
